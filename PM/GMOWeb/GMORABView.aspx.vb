...
Partial Class GMORABView
    ...
    Public Function Select_Recipient() As DataSet
        ...
        Try
            Dim y As String
            If Trim(Me.Session("_zonecode")) = "VISMIN" Then
                y = "select memopath from recipientslist where MemoFrom ='" + Trim(Me.Session("_MemoFrom")) + "' and subject='" + Trim(Me.Session("_Subject")).Replace("'", "''").Replace("&#241;", "ñ") + "' and CircularNumber='" + Trim(Me.Session("_Circular")) + "' and ZoneCode = 'VISMIN'"
                ds = Execute_Dataset(y)
            End If
            If Trim(Me.Session("_zonecode")) = "LUZON" Then
                y = "select memopath from recipientslist where MemoFrom ='" + Trim(Me.Session("_MemoFrom")) + "' and subject='" + Trim(Me.Session("_Subject")).Replace("'", "''").Replace("&#241;", "ñ") + "' and CircularNumber='" + Trim(Me.Session("_Circular")) + "' and ZoneCode = 'LUZON'"
                ds = Execute_Dataset(y)
            End If
    ...
    Public Function Select_Trxnumber() As DataSet
        ...
        Try
            ...
            ElseIf Trim(Me.Session("_task")) = "BM/BOSMAN" Or Trim(Me.Session("_task")) = "Bm/Bosman" Then
                ...
                ds = Execute_Dataset(y)
            Else
                y = "select distinct md.division, md.trxnumber from memodivision as md inner join recipientslist as rl on md.trxnumber = rl.trxnumber where rl.CircularNumber = '" + Trim(Me.Session("_Circular")) + "' and rl.subject = '" + Trim(Me.Session("_Subject")).Replace("'", "''") + "' and md.division = '" + Trim(Me.Session("_fullname")) + "'"
                ds = Execute_Dataset(y)
            End If
        Catch ex As Exception
            MsgBox(ex.Message)
    ...
    Public Function Update_ReceivedMemo() As DataSet
        ...
        Else
            ...
            Dim B As String = "update memodivision set confirmedReceived='1', confirmedDate='" + CMemo.Trim + "' where division = '" + BN.Trim + "' and trxnumber = '" + TN.Trim + "'"
    ...
    Private Sub SavePDFDoc(ByVal PDF As PDFCreatorPilotLib.PDFDocument4, ByVal FileName As String)
        If Trim(Me.Session("_zonecode")) = "VISMIN" Then
            'PDF.SaveToFile("D:\Silvher's Vb.net 08\GMOWeb\MemoImageVismin\" & FileName, False)
            PDF.SaveToFile("C:\GMOWeb\MemoImageVismin\" & FileName, False)
            PDF.SaveToFile(sDest2 & FileName, False)
            PDF = Nothing
        End If
        If Trim(Me.Session("_zonecode")) = "LUZON" Then
            'PDF.SaveToFile("D:\Silvher's Vb.net 08\GMOWeb\MemoImageLuzon\" & FileName, False)
